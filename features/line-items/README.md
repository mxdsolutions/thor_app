...
- **`LineItemsTable`** — Full table with add row, inline editing, delete, and totals footer
  - `mode: "live"` — Calls API endpoints for CRUD; used in JobSideSheet, OpportunitySideSheet
  - `mode: "draft"` — Parent manages array via `onItemsChange`; used in CreateOpportunityFromLeadModal, CreateJobFromOpportunityModal
...
- `/api/opportunity-line-items` — GET, POST, PATCH, DELETE
