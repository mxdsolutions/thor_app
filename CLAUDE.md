...
- Use Lucide (`lucide-react`) for **all** icons. Import as `import { Foo } from "lucide-react"`. No other icon library is in the codebase — Tabler was fully migrated out.
- **Typography:** body text is Inter (`--font-inter`, wired to `--font-sans`) and is the default everywhere, including `h1`–`h6` (the base layer just adds `font-weight: 600` and `letter-spacing: -0.011em`). For statement/display headings — dashboard welcome, hero headlines, marketing — opt into Bricolage Grotesque with `font-statement` directly on the heading element (e.g. `<h2 className="font-statement text-3xl font-extrabold tracking-tight">`). The dashboard overview page is the canonical reference for the look. Paladins (`font-paladins`) is reserved exclusively for the THOR™ wordmark. Mono text uses `font-mono` (IBM Plex Mono) for reference IDs and code-like text.
- **Radius scale** is intentionally collapsed: `rounded-sm`/`rounded-md` → 4px (fine details), `rounded-lg`/`rounded-xl`/`rounded-2xl`/`rounded-3xl` → 8px (surfaces and interactive controls). All "larger" tokens render the same — pick by intent. Don't introduce arbitrary values like `rounded-[12px]`. The one exception is the device-frame mockup in `components/platform-admin/builder/BuilderPreviewCanvas.tsx`.
- Buttons: `rounded-lg` (8px) — this is the default. Reserve `rounded-full` for avatars and pill badges only. Cards: `rounded-2xl border bg-card shadow-sm` (also renders at 8px).
